...
#define ADIO GPIOC        //?จ??AD??????????I/O????ื้??  
#define DATA GPIO_Pin_11	  //?จ??AD???????????????????????? 
#define CLK  GPIO_Pin_12	  //?จ??AD???????ฑ???????????????? 
#define ADCLK RCC_APB2Periph_GPIOC  //?จ??AD?????๙??????I/O???????ฑ??? 
...
{ 
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; //CLK?????จ?????????? 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
	GPIO_Init(ADIO,&GPIO_InitStructure); 

		GPIO_InitStructure.GPIO_Pin = DATA; 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//DATA?????จ?????????? 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
	GPIO_Init(ADIO,&GPIO_InitStructure);	
} 
